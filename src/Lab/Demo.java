...
public class Demo {
    ...
    private static int getSum(int[][] inputMatrix, int row, int col, int wrongValue) {
        ...
        if (row - 1 >= 0 && inputMatrix[row - 1][col] != wrongValue) { //не е първи ред и горното число не е грешно
            sum += inputMatrix[row - 1][col];
        }
        if (row + 1 < inputMatrix.length && inputMatrix[row + 1][col] != wrongValue) { //не е последен ред и долното число не е грешно
            sum += inputMatrix[row + 1][col];
        }
        if (col - 1 >= 0 && inputMatrix[row][col - 1] != wrongValue) { //не е първа колона и лявото число не е грешно
            sum += inputMatrix[row][col - 1];
        }
        if (col + 1 < inputMatrix[row].length && inputMatrix[row][col + 1] != wrongValue) { //не е последна колона и дясното число не е грешно
            sum += inputMatrix[row][col + 1];
        }
